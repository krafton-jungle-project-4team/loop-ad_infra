...
| `LOOPAD_DEMO_DISPATCH_RECIPIENTS` | Dashboard API 전용 Secrets Manager JSON array |
...
| `LOOPAD_ALLOWED_ORIGINS` | Event Collector 전용. `https://demo-shoppingmall.dev.loop-ad.org` |
...
| `LOOPAD_BRAND_CONTEXT_BASE_PREFIX` | Decision API 전용 private source prefix `brand-context/` |
| `LOOPAD_OPENAI_API_KEY` | OpenAI secret `api_key` |
| `LOOPAD_GEMINI_API_KEY` | Gemini secret `api_key` |
| `LOOPAD_DEMO_DISPATCH_RECIPIENTS` | JSON 배열 secret string |

...
각 repo에 넣을 caller workflow 템플릿은 `docs/github-actions/templates/`에 둡니다.

| 템플릿 | 대상 |
|---|---|
| `event-collector.ecs-deploy.yml` | Event Collector ECS service |
| `ad-context-projector.ecs-deploy.yml` | Ad Context Projector ECS service |
| `ad-decision-api.ecs-deploy.yml` | Ad Decision API ECS service |
| `dashboard-api.ecs-deploy.yml` | Dashboard API ECS service |
| `recommendation.ecs-deploy.yml` | Recommendation ECS service |
| `dashboard-web.frontend-deploy.yml` | Dashboard Web S3/CloudFront deploy |

템플릿은 각 애플리케이션 repo의 `.github/workflows/deploy.yml`로 복사해서 사용합니다. `aws_role_arn`, FE의 `s3_bucket`, `cloudfront_distribution_id`는 실제 AWS 리소스 값으로 교체해야 합니다. 모든 workflow input은 fallback 없이 명시적으로 받아서, 누락되면 즉시 실패합니다.

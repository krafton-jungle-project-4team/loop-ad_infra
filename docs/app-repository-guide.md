...
ECS service는 private subnet에서 실행됩니다. 내부 서비스 URL은 Cloud Map 이름을 사용해도 됩니다. 예를 들어 Dashboard API 개발자는 Decision 호출 주소로 아래 값을 사용할 수 있습니다.

```text
http://decision.dev.loop-ad.local:80
```
...
외부 사용자에게 노출하는 URL은 S3 bucket URL이 아니라 CloudFront public base URL을 사용합니다. CloudFront origin path가 내부 S3 prefix인 `genai/generated/`에 매핑되므로 앱은 외부 URL을 만들 때 내부 prefix를 붙이지 않습니다.

```text
https://gen-ai.asset.dev.<public-domain>/<object-name>
```
...
| `LOOPAD_GENAI_GENERATED_ASSETS_PUBLIC_BASE_URL` | Plain | `https://gen-ai.asset.dev.loop-ad.org` | 외부 사용자에게 반환할 GenAI 생성물 public base URL입니다. |
...
| 이름 | 종류 | 출처 | 설명 |
|---|---|---|---|
| `LOOPAD_OPENAI_API_KEY` | Secret | SSM SecureString 또는 Secrets Manager | Decision에서 OpenAI API를 호출할 때 사용합니다. |
...
Advertisement API는 ALB를 통해 공개 API 요청을 받고 Aurora/Redis를 사용합니다.
...
| `LOOPAD_REDIS_URL` | Plain 또는 Secret | Yes | advertisement cache endpoint |

HTTP health check는 `/health`에서 `200-399`를 반환해야 합니다.

#### Dashboard API

Dashboard API는 ALB를 통해 dashboard API 요청을 받고 Aurora/ClickHouse/Decision/external webhook을 사용합니다.
...
| `LOOPAD_GENAI_GENERATED_ASSETS_PUBLIC_BASE_URL` | Plain | Yes | `https://gen-ai.asset.dev.loop-ad.org` |
| `LOOPAD_DECISION_URL` | Plain | Yes | `http://decision.dev.loop-ad.local:80` |
...
Decision은 private service이며 Dashboard API에서 Cloud Map DNS로 호출합니다.
...
| `LOOPAD_AURORA_USERNAME` | Secret | Yes | Aurora username |
| `LOOPAD_AURORA_PASSWORD` | Secret | Yes | Aurora password |
| `LOOPAD_CLICKHOUSE_URL` | Plain | Yes | decision feature 조회용 ClickHouse endpoint |
| `LOOPAD_CLICKHOUSE_USERNAME` | Secret | Yes | ClickHouse username |
| `LOOPAD_CLICKHOUSE_PASSWORD` | Secret | Yes | ClickHouse password |
| `LOOPAD_DATA_STORAGE_BUCKET` | Plain | Yes | GenAI 생성물 DataStorage bucket |
| `LOOPAD_GENAI_GENERATED_ASSETS_PREFIX` | Plain | Yes | `genai/generated/` |
